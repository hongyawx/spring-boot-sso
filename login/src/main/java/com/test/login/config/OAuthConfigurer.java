...
public class OAuthConfigurer extends AuthorizationServerConfigurerAdapter {
    ...
    public void configure(ClientDetailsServiceConfigurer clients)
            throws Exception {
        // authorizedGrantTypes 允许的授权类型 , scopes 允许的授权范围  ,secret 为   client_secret
        clients.inMemory().withClient("ssoclient").secret("ssosecret")
                .autoApprove(true)    // 自动确认授权
                .authorizedGrantTypes("refresh_token","implicit").scopes("openid");
        //todo .authorizedGrantTypes("authorization_code", "refresh_token").scopes("openid");
    }
